...
export default function(data) {
    ...
    for (let page = 1; page <= totalPages; page++) {
       ...
       // console.log("body: ", body);
        ...
        if (!check(res, {'é status 200': (r) => r.status === 200})) {
            console.error(`Erro ao processar página ${page}. Status: ${res.status} | Response: ${res.body}`);
            break;
        }

        if (numItems < pageLength) {
            console.log(`VU ${__VU} - Página ${page}: Recebeu ${numItems} credenciais, esperado era ${pageSize}.`);
            break; // 
        }
