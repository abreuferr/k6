...
export default function() {
    const TotalUsers = 45; // Número existente de usuários
    const TotalUsersRegister = 50; // Número desejado de usuários
    const UsersParaCriar = TotalUsersRegister - TotalUsers; // Usuários a serem criados
    const UsersCriadosPorIteracao = Math.ceil(UsersParaCriar / (options.vus * options.iterations));

    const vuId = __VU;  // ID do Usuário Virtual
    const iterId = __ITER;  // ID da iteração
    const usersCriadosTotal = (vuId - 1) * UsersCriadosPorIteracao + iterId * (options.vus * UsersCriadosPorIteracao);
    ...
    for (let i = 0; i < UsersCriadosPorIteracao; i++) { // A cada iteração será acrescentado um usuário até completar o "TotalUsersRegister"
        ...
        const userId = usersCriadosTotal + i + TotalUsers + 1;  // Adiciona o número total de usuários existentes

        // Lógica para criar um usuário
        const username = `User${userId}`;
        console.log(`Criando usuário de ID: ${userId}, Nome: ${username}`);
...
console.log(`CRIADO USER: ${userId}, Nome: ${username}`);
}
